	                           ...
	                           <tbody>
                                      ...
                                      <div class="userDatatable-content">
                                        <div class="custom-control custom-switch switch-info switch-md ">
                                                <input type="checkbox" data-id="{{ $value1->id }}" class="custom-control-input active_inactive"  id="switch-dynamic-badge{{ $value1->id }}" {{ $value1->status == 1 ? 'checked' : '' }}>
                                                <label class="custom-control-label" for="switch-dynamic-badge{{ $value1->id }}"></label>
                                            </div>
                                        </div>
                                   </td>
																	 <td>
																			<div class="userDatatable-content d-flex">
																				 <a  class="custom_btn_primary text-primary update_record mr-2"  data-id="{{$value1->id}}" data-name="{{$value1->name}}" data-toggle="tooltip" title="update"> <i class="la la-pen-alt p-2"></i> </a>
																				 <a  class="custom_btn_danger  text-danger destroy_record" data-id="{{$value1->id}}"  data-toggle="tooltip" title="Delete"><i class="la la-trash p-2"></i></a>
																			</div>
																	 </td>
																</tr>
															@empty
...
@section('scripts')
	...
	$(document).on('click', '#papers_form', function(){
		...
		$.ajax({
			...
			url: url + '/support/tables/papers_store',
			data:{
				name:name,
			},
			dataType: "json",
    ...
    $.ajax({
      type: "POST",
      headers: { 'X-CSRF-TOKEN': csrf_token },
      url: url + '/support/tables/papers_remove',
      data:{
        id:id,
      },
      dataType: "json",
    ...
    $.ajax({
      type: "POST",
      headers: { 'X-CSRF-TOKEN': csrf_token },
      url: url + '/support/tables/papers_update',
      data:{
        id:id,
        name:name,
      },
	...
	$('.active_inactive').change(function () {
			...
			$.ajax({
				type: "GET",
				url: url + '/support/tables/papers_status_update',
				data:{
					id:id,
					status:status,
				},
